...
#		  
#    	Copyright (C) 2015-2023  Douglas Chesters
#
	...
	if( $push_node =~ /INTERNAL_NODE_\d/ ) 
		...
		#	print "\t:$count_barcodes_this_backbone_child barcodes for this backbone term:@splitmembers\n";
			...
			if($barcode_terminals_string =~ s/\t+/,/g)
				...
				if($constreaint =~ /^[A-Z][a-z]+$/){$barcode_terminals_string_tax = $barcode_terminals_string_tax . $constreaint};
...
#	print "\nFT relational contraint, bipart:\n";
	...
	if(scalar @new_collapsed_node >= 2){print "\tA. writing polytomie to constraint tree.\n"};
...
# print name only:
return("$most_inclusive_rank:$most_inclusive_name");
# or lineage:
#print "\treturning:($most_inclusive_lineage)\n";
# return($most_inclusive_lineage);
